...
import { Plus, Package, DollarSign, Activity, Search, Filter, X, Loader2, CheckCircle2 } from 'lucide-react';
    ...
    <div className="min-h-screen bg-[#F1F2F4] text-[#1A1C1D] font-sans pb-24">
      ...
      <nav className="bg-white border-b border-[#E1E3E5] px-8 py-3 flex justify-between items-center sticky top-0 z-40">
        ...
        <div className="flex items-center gap-3">
            <div className="bg-[#EBEBEB] h-8 w-8 rounded-full border border-[#E1E3E5]"></div>
        </div>
      </nav>
      ...
      <main className="max-w-[1200px] mx-auto mt-8 px-8">
        ...
        <div className="flex justify-between items-end mb-6">
          ...
          <button 
            ...
            className="bg-[#008060] hover:bg-[#006e52] text-white px-4 py-2 rounded-md text-sm font-semibold transition-colors shadow-sm flex items-center gap-2"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          <div className="bg-white p-4 rounded-xl border border-[#E1E3E5] shadow-sm">
            <div className="flex items-center gap-2 text-[#6D7175] text-xs font-medium mb-1 uppercase tracking-wider">
              <Package size={16}/> Total Inventory
            </div>
            <div className="text-xl font-bold">{products.length}</div>
          </div>
          <div className="bg-white p-4 rounded-xl border border-[#E1E3E5] shadow-sm">
            <div className="flex items-center gap-2 text-[#6D7175] text-xs font-medium mb-1 uppercase tracking-wider">
              <Activity size={16}/> Active Storefronts
            </div>
            <div className="text-xl font-bold">1</div>
          </div>
          <div className="bg-white p-4 rounded-xl border border-[#E1E3E5] shadow-sm">
            <div className="flex items-center gap-2 text-[#6D7175] text-xs font-medium mb-1 uppercase tracking-wider">
              <CheckCircle2 size={16} className="text-[#008060]"/> Vault Status
            </div>
        ...
        <div className="bg-white rounded-xl border border-[#E1E3E5] shadow-sm overflow-hidden">
          <div className="p-4 border-b border-[#E1E3E5] flex gap-2 bg-white">
            <div className="relative flex-grow">
              ...
              <input type="text" placeholder="Filter products" className="w-full pl-10 pr-4 py-2 bg-white border border-[#8C9196] rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-[#008060] focus:border-transparent transition-all" />
            </div>
            <button className="px-4 py-2 border border-[#8C9196] rounded-md text-sm font-semibold hover:bg-[#F6F6F7] flex items-center gap-2 transition-colors">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse min-w-[600px]">
              ...
              <tbody className="divide-y divide-[#E1E3E5]">
                {isLoading ? (
                  <tr><td colSpan={4} className="py-20 text-center"><Loader2 className="animate-spin mx-auto text-[#6D7175]" /></td></tr>
                ) : products.length === 0 ? (
                  <tr><td colSpan={4} className="py-20 text-center text-[#6D7175] text-sm">No products found in the vault. Add one to get started.</td></tr>
                ) : products.map((p) => (
                  <tr key={p.id} className="hover:bg-[#F6F6F7] transition-colors cursor-pointer group">
                    ...
                    <td className="px-6 py-4">
                      <span className="bg-[#E3F1DF] text-[#008060] text-[11px] px-2.5 py-0.5 rounded-full font-bold">Active</span>
                    </td>
